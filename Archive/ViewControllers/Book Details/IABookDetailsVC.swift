...
class IABookDetailsVC: UIViewController, UITableViewDelegate, UITableViewDataSource, IALoadingViewProtocol, UICollectionViewDelegate, UICollectionViewDataSource {
                    ...
                    if let collections = dictionary["collection"] as? [String] {
                        for collection in collections {
                            ...
                            itemManager.itemMetadataDetails(collection, completion: { dictionary in
                                book.collections.append(IAArchiveItem(dictionary: dictionary))
    ...
    func showChapters() {
        if let file = self.book!.file {
            self.chapters = file.chapters.sort({$0.name<$1.name})
        }
    ...
    func populateData() {
        if let book = book {
            ...
            if let authors = authors() where authors.count != 0 {
                self.parameters.append(Parameter(title: "Author", values: authors, type: .Author))
            }
            ...
            if let subjects = subjects() where subjects.count != 0 {
                self.parameters.append(Parameter(title: "Subjects", values: subjects, type: .Subject))
            }
        }
        self.parametersTableView.reloadData()
    ...
    func sortedCollections()->[IAArchiveItem]? {
    ...
    func showReader(item: IAArchiveItem, atChapterIndex chapterIndex :Int = 0) {
